package controller

import (
	"context"
	"log"
	configs "my-kanban/config"
	"my-kanban/helpers"
	"my-kanban/models"
	"net/http"
	"time"

	"github.com/gin-gonic/gin"
	"go.mongodb.org/mongo-driver/bson"
	"go.mongodb.org/mongo-driver/mongo"
	"golang.org/x/crypto/bcrypt"
)

var userCollection *mongo.Collection = configs.GetCollection(configs.DB, "users") //initial mongo CLient and create database name is kanban

func HashPassword(password string) string {
	bytes, err := bcrypt.GenerateFromPassword([]byte(password), 14) //helloword --hashed-->$2a$14$IzdvYkAsnJPF8L/Ok.giyug/j/qDg10YqjIpB6X3hm9kCztJLtmfy
	if err != nil {
		log.Fatal("error hash password:", err)
	}

	return string(bytes)
}

func VeryfyPassword(userPassword string, providedPassword string) (bool, string) {
	err := bcrypt.CompareHashAndPassword([]byte(providedPassword), []byte(userPassword))
	check := true
	msg := ""
	if err != nil {
		msg = "user or password is incorrect"
		check = false
	}
	//check, msg := VeryfyPassword("helloword", "$2a$14$IzdvYkAsnJPF8L/Ok.giyug/j/qDg10YqjIpB6X3hm9kCztJLtmfy")
	//=>check=true && msg=""
	return check, msg
}

//CreateUser is used to create new user
func SignUp() gin.HandlerFunc {
	return func(c *gin.Context) {
		ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
		var user models.SignUpInput

		if err := c.BindJSON(&user); err != nil { // check json empty send from client
			c.JSON(http.StatusBadRequest, gin.H{"error_signup": err.Error()})
			defer cancel()
			return
		}

		//After BindJSON user become pointer
		//You can access value using *user.value ex:*user.email

		count, err := userCollection.CountDocuments(ctx, bson.M{"email": user.Email})
		defer cancel()
		if err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"Error": "error occured when cheking email"})
			return
		}
		if count > 0 {
			c.JSON(http.StatusInternalServerError, gin.H{"error": "This email already exists"})
			return
		}

		idGetKanban := "6387347ca92496eddbc3a110"

		password := HashPassword(user.Password)
		user.Password = password
		user.IdGetKanban = idGetKanban

		acessToken, refreshToken, err := helpers.GenerateAllTokens("6387347ca92496eddbc3a110")

		if err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error when generate token": err.Error()})
			return
		}

		user.AccessToken = acessToken
		user.RefreshToken = refreshToken

		_, insertErr := userCollection.InsertOne(ctx, user)

		if insertErr != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"Error when create user": insertErr.Error()})
			return
		}

		defer cancel()
		c.JSON(http.StatusOK, gin.H{"message": "success"})
	}
}

//Login is used to login user

func Login() gin.HandlerFunc {
	return func(c *gin.Context) {
		ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
		var user models.SignInInput
		var foundUser models.DBResponse

		if err := c.BindJSON(&user); err != nil {
			c.JSON(http.StatusBadRequest, gin.H{"error bind json": err.Error()})
			defer cancel()
			return
		}
		err := userCollection.FindOne(ctx, bson.M{"email": user.Email}).Decode(&foundUser)
		defer cancel()
		if err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": "User not found"})
			return
		}

		passwordIsValid, msg := VeryfyPassword(user.Password, foundUser.Password) // if password not match
		if !passwordIsValid {
			c.JSON(http.StatusInternalServerError, gin.H{"error": msg})
			return
		}

		token, refreshToken, _ := helpers.GenerateAllTokens(" 6387347ca92496eddbc3a110")
		helpers.UpdateAllTokens(token, refreshToken, "6387347ca92496eddbc3a110")

		c.JSON(http.StatusOK, foundUser) // send json to client
	}
}

// func GetKanbanWhenUserLogin() gin.HandlerFunc {
// 	return func(c *gin.Context) {

// 		ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second) //set timeout for request to database

// 		var board models.AutoGenerated

// 		var err error
// 		var user models.User
// 		var foundUser models.User

// 		if err := c.BindJSON(&user); err != nil {
// 			defer cancel()
// 			c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
// 			return
// 		}

// 		err = userCollection.FindOne(ctx, bson.M{"email": user.Email}).Decode(&foundUser)
// 		if err != nil {
// 			defer cancel()
// 			c.JSON(http.StatusBadRequest, gin.H{"error_email": err.Error()})
// 			return
// 		}

// 		err = kanbanCollection.FindOne(ctx, bson.M{"id_owner": foundUser.IdGetKanban}).Decode(&board)
// 		if err != nil {
// 			defer cancel()
// 			c.JSON(http.StatusBadRequest, gin.H{"error_getboard": err.Error()})
// 			return
// 		}

// 		defer cancel()
// 		defer c.JSON(http.StatusOK, board)
// 	}
// }
