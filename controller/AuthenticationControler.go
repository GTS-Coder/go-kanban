...
var userCollection *mongo.Collection = configs.GetCollection(configs.DB, "users") //initial mongo CLient and create database name is kanban
...
func SignUp() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if err := c.BindJSON(&user); err != nil { // check json empty send from client
			c.JSON(http.StatusBadRequest, gin.H{"error_signup": err.Error()})
			defer cancel()
			return
		}

		//After BindJSON user become pointer
